...
import { getGroupMember, banUser } from "@/lib/queries"
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (target?.role === GroupRole.Owner) return NextResponse.json({ error: "Cannot ban the group owner" }, { status: 400 })

  const ban = await banUser(params.id, parsed.data.userId)
