...
import { Check, Copy, UserMinus, UserPlus, UserRoundPlus, Users, X } from "lucide-react"
...
export default function FriendsPage() {
  ...
  const sendRequestMutation = useMutation({
    mutationFn: (receiverId: string) => httpClient.post("/api/friends/request", { receiverId }),
  })

  const respondMutation = useMutation({
    mutationFn: ({ requestId, action }: { requestId: string; action: "accepted" | "declined" }) =>
      httpClient.put(`/api/friends/request/${requestId}`, { action }),
  })

  const removeFriendMutation = useMutation({
    mutationFn: (friendId: string) => httpClient.delete("/api/friends", { friendId }),
  })
  ...
  async function handleSearch(q: string) {
    ...
    setSearching(true)
    try {
      const data = await httpClient.get<User[]>(`/api/friends/search?q=${encodeURIComponent(q)}`)
  ...
  async function handleSendRequest(receiverId: string, onDone?: () => void) {
    await promise(sendRequestMutation.mutateAsync(receiverId), {
  ...
  async function handleAddByCode() {
    ...
    try {
      const user = await httpClient.get<User>(`/api/friends/by-code?code=${encodeURIComponent(friendCodeInput.trim())}`)
  ...
  async function handleRespond(requestId: string, action: "accepted" | "declined") {
    await promise(respondMutation.mutateAsync({ requestId, action }), {
      loading: action === "accepted" ? "Accepting..." : "Declining...",
      success: action === "accepted" ? "Request accepted!" : "Request declined",
      error: (err: Error) => err.message,
    })
    queryClient.invalidateQueries({ queryKey: ["/api/friends/requests"] })
  ...
  async function handleRemoveFriend(friendId: string) {
    ...
    if (!ok) return
    await promise(removeFriendMutation.mutateAsync(friendId), {
      loading: "Removing...",
